...
public class UcRedisTokenStore implements TokenStore {
    ...
    // 扩展用户名关联token
    private static final String ONLY_UNAME_TO_ACCESS = "only_uname_to_access:";
    ...
    public UcRedisTokenStore(RedisConnectionFactory connectionFactory) {
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        // 扩展
        byte[] unameKey = serializeKey(ONLY_UNAME_TO_ACCESS + getUnameKey(authentication));
        ...
        try {
            ...
            if (!authentication.isClientOnly()) {
                ...
                conn.sAdd(unameKey, serializedAccessToken);
            }
    ...
    private static String getUnameKey(OAuth2Authentication authentication) {
    ...
    public void removeAccessToken(String tokenValue) {
        ...
        try {
            ...
            if (authentication != null) {
                ...
                byte[] clientId = serializeKey(CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId());
                byte[] onlyUnameKey = serializeKey(ONLY_UNAME_TO_ACCESS + getUnameKey(authentication));
                conn.openPipeline();
                conn.del(authToAccessKey);
                conn.sRem(unameKey, access);
                conn.sRem(clientId, access);
                conn.sRem(onlyUnameKey, access);
    ...
    public Collection<OAuth2AccessToken> findTokensByUserName(String userName) {
        byte[] approvalKey = serializeKey(ONLY_UNAME_TO_ACCESS + userName);
